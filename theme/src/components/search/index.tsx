...
export const Search: FunctionComponent = () => {
  ...
  const search = event => {
    ...
    const lunrIndex = window.__LUNR__['en']; // tslint:disable-line
    setResults(
  ...
  return (
    <>
      <NavMenuItem>
        <ToggleSearchButton role={`button`} aria-label={`Toggle search`} onClick={toggleSearch}>
          <FaSearch />
        </ToggleSearchButton>
      </NavMenuItem>
      ...
      {isOpen && (
        <SearchBox open={isOpen} ref={searchRef}>
          <SearchInput placeholder={`Search...`} autoFocus={true} ref={inputRef} value={query} onChange={search} onKeyDown={handleKey} />
